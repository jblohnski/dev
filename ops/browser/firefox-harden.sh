...
#
# Purpose:
#   Apply the arkenfox user.js from this repo/zip to your Firefox profile(s),
#   then append a small set of overrides so:
#     - New Tab / Home stays Firefox default (background + shortcuts + search)
#     - Cookies default to session-only, but Google logins remain persistent
#
# Usage:
...
#
# Notes:
#   - This script modifies files inside your Firefox profile directory.
#   - It makes timestamped backups of any existing user.js / prefs.js / permissions.sqlite.
#   - Google cookie exceptions are applied by writing entries into permissions.sqlite.
#   - Works on macOS/Linux. (Windows is out of scope for this single bash file.)
set -euo pipefail
...
ARKENFOX_USERJS
  ...
  # Append overrides at end (single file outcome)
  cat >>"$target" <<'HI_OVERRIDES'

/*** highinquires overrides (keep functionality) ***/
// Keep Firefox "Default" New Tab / Home page with background + shortcuts + search
user_pref("browser.startup.page", 1);                      // 0=blank, 1=home, 2=last session, 3=resume previous
user_pref("browser.startup.homepage", "about:home");
user_pref("browser.newtabpage.enabled", true);
user_pref("browser.newtab.preload", true);

// New Tab content: keep search + top sites, remove sponsored content
user_pref("browser.newtabpage.activity-stream.showSearch", true);
user_pref("browser.newtabpage.activity-stream.feeds.topsites", true);
user_pref("browser.newtabpage.activity-stream.feeds.section.topstories", false); // Pocket/Top Stories
user_pref("browser.newtabpage.activity-stream.showSponsored", false);
user_pref("browser.newtabpage.activity-stream.showSponsoredTopSites", false);

// Cookies: default to "session only" while allowing persistent login for Google (exceptions added by script)
...
# Robustly insert cookie allow entries into permissions.sqlite
# Handles both newer (origin) and older (host) schemas.
allow_cookie_origin() {
  local db="$1"
  local origin="$2"

  command -v sqlite3 >/dev/null 2>&1 || {
    warn "sqlite3 not found; cannot add Google cookie exceptions automatically."
    warn "You can still set them manually in Firefox: Settings -> Privacy & Security -> Cookies and Site Data -> Manage Exceptions."
    return 0
  }

  # Determine schema
  local has_origin=""
  has_origin="$(sqlite3 "$db" "PRAGMA table_info(moz_perms);" | awk -F'|' '{print $2}' | grep -x "origin" || true)"
  local now_us
  now_us="$(python3 - <<'PY'
import time
...
apply_profile() {
  ...
  # Write user.js
  write_userjs "$prof_dir/user.js"

  # Add Google cookie exceptions (persistent cookies) while lifetimePolicy=2 makes everything else session-only
  if [ -f "$prof_dir/permissions.sqlite" ]; then
    allow_cookie_origin "$prof_dir/permissions.sqlite" "https://accounts.google.com"
    allow_cookie_origin "$prof_dir/permissions.sqlite" "https://google.com"
    allow_cookie_origin "$prof_dir/permissions.sqlite" "https://www.google.com"
    allow_cookie_origin "$prof_dir/permissions.sqlite" "https://mail.google.com"
    allow_cookie_origin "$prof_dir/permissions.sqlite" "https://drive.google.com"
    allow_cookie_origin "$prof_dir/permissions.sqlite" "https://docs.google.com"
    allow_cookie_origin "$prof_dir/permissions.sqlite" "https://calendar.google.com"
  else
    warn "permissions.sqlite not found in $prof_dir (profile may not be initialized yet)."
    warn "Start Firefox once, quit, then re-run this script to apply Google exceptions."
  fi
